...
/*@ 
  ...
  ensures \forall int i; 0 <= i < \old(pIp->countMode) ==> (pIp->mController[i] == \old(pIp->mController[i]) || pIp->mDSSData == \old(pIp->mDSSData));
  assigns pIp->countMode, pIp->mDSSData, pIp->mController[0];
*/
void ModeControlFun(ModeControl *pIp)
{
    ...
    /*@
      loop invariant \true;
      loop assigns flgModetmp1, pIp->countMode;
      loop variant 1;
    */
