...
/*@ 
  ...
  requires \valid(pIp->pGyroData->wa + (0..(pIp->pGyroData->JoinTotal - 1)));
  requires \valid(pIp->pGyroData->W + (0..2));
  requires \valid(pIp->pGyroData->Rtemp);
  requires pIp->pGyroData->JoinTotal >= 0;
  ensures \forall int i; 0 <= i < 3 ==> pIp->pGyroData->W[i] == (pIp->pGyroData->JoinTotal >= 3 ? \old(pIp->pGyroData->W[i]) : 0.0f);
*/
