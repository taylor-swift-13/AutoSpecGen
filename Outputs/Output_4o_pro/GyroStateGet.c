...
/*@ 
  ...
  ensures \forall integer i; 0 <= i < pIp->NumGyro ==> \valid(pIp->wa + i);
  ensures \forall integer i; 0 <= i < pIp->NumGyro ==> \valid(pIp->stateFlag + i);
...
void GyroStateGetFun(GyroStateGet *pIp)
{
    ...
    /*@ assert \valid(gyroRecvData + (0 .. 29)); */
    if ( (gyroRecvData[0] == 0xEB) && (gyroRecvData[1] == 0x91) )
    {
        /*@ 
          loop invariant 0 <= i <= pIp->NumGyro;
          loop invariant \forall integer j; 0 <= j < i ==> \valid(pIp->wa + j);
          loop invariant \forall integer j; 0 <= j < i ==> \valid(pIp->stateFlag + j);
