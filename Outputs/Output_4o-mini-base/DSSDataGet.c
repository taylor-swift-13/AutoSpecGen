...
/*@ 
  requires pIp != NULL;
  requires pIp->pDSSData != NULL;
  ensures pIp->pDSSData->stateFlag_A == (if (stateFlag & 0x1) == 0x1 then 1 else 0);
  ensures pIp->pDSSData->stateFlag_B == (if (stateFlag & 0x2) == 0x2 then 1 else 0);
  ensures pIp->pDSSData->flgSP == ((ADDR_READ(0xE000) >> 5) & 0x01);
  ensures (stateFlag > 0) ==> (ADDR_WRITE(0xA100, (0xC + si)) && 
           (tmp_ad_start == ADDR_READ(0xA000)) &&
           (tmp_lo4 == ADDR_READ(0xA003)) &&
           (tmp_hi8 == ADDR_READ(0xA001)) &&
           (wd == ((tmp_hi8 << 4) & 0x0FF0) | ((tmp_lo4 >> 4) & 0xF)));
  ensures (si == 0) ==> (pIp->pDSSData->royaw == DiviationToFloat_ret);
  ensures (si == 1) ==> (pIp->pDSSData->piyaw == DiviationToFloat_ret);
*/
void DSSDataGetFun(DSSDataGet *pIp)
{
	...
	float32 DiviationToFloat_ret; // Declare variable here
