...
void GyroStateGetFun(GyroStateGet *pIp)
{
    ...
    /*@ requires \valid(gyroRecvData + (0..29));
        ensures (gyroRecvData[0] == 0xEB && gyroRecvData[1] == 0x91) ==> 
                (\forall integer i; 0 <= i < pIp->NumGyro ==> 
                \valid(pIp->wa + i) && \valid(pIp->stateFlag + i));
    */
    if ( (gyroRecvData[0] == 0xEB) && (gyroRecvData[1] == 0x91) )
    {
        /*@ loop invariant 0 <= i <= pIp->NumGyro;
            loop invariant \valid(pIp->wa + (0..pIp->NumGyro-1));
            loop invariant \valid(pIp->stateFlag + (0..pIp->NumGyro-1));
