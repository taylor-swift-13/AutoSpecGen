...
/*@ 
  ...
  requires \forall integer i; 0 <= i < 3 ==> \valid(pIp->pGyroData->Rtemp[i] + (0 .. 4));
  assigns pIp->pGyroData->W[0 .. 2];
  ensures \forall integer i; 0 <= i < 3 ==> (pIp->pGyroData->JoinTotal >= 3 ==> pIp->pGyroData->W[i] == \old(pIp->pGyroData->W[i]) || pIp->pGyroData->W[i] == 0.0f);
*/
void CalculateGyroDgFun(CalculateGyroDg *pIp)
{
    ...
    /*@ loop invariant 0 <= j <= 5;
        loop invariant \forall integer i; 0 <= i < j ==> tmpwa[i] == 0;
    ...
    */
    ...
    /*@ loop invariant 0 <= j <= pIp->pGyroData->JoinTotal;
        loop invariant \forall integer i; 0 <= i < j ==> tmpwa[i] == pIp->pGyroData->wa[pIp->pGyroData->SignFlag[i]];
