...
/*@ 
  ...
  ensures pIp->bAbleInv == (pIp->src[0] * pIp->inv[0] + pIp->src[1] * pIp->inv[3] + pIp->src[2] * pIp->inv[6]) > FLT32_ZERO;
  ensures \forall integer i; 0 <= i < 9 ==> (pIp->bAbleInv ? pIp->inv[i] == \old(pIp->inv[i]) / (pIp->src[0] * pIp->inv[0] + pIp->src[1] * pIp->inv[3] + pIp->src[2] * pIp->inv[6]) : pIp->inv[i] == pIp->src[i]);
