...
/*@ 
  ...
  requires \valid(pIp->pSCtrl + (0..2)); // Ensure that the entire array is valid
  requires pIp->max >= 0; // Assuming max should be non-negative
  assigns pIp->pSCtrl[0].Ud, pIp->pSCtrl[1].Ud, pIp->pSCtrl[2].Ud;
  ensures pIp->pSCtrl[0].Ud == \old(pIp->pSCtrl[0].Ud); // Ensure postconditions reflect the new values
  ensures pIp->pSCtrl[1].Ud == \old(pIp->pSCtrl[1].Ud); 
  ensures pIp->pSCtrl[2].Ud == \old(pIp->pSCtrl[2].Ud);
*/
