...
/*@ 
  ...
  assigns pIp->pGyroData->JoinTotal, pIp->pGyroData->gyroStatus0, pIp->pGyroData->SignFlag[..];
*/
void GyroChooseFun(GyroChoose *pIp)
{
    ...
    /*@ loop invariant 0 <= i <= 9;
        loop assigns pIp->pGyroData->JoinTotal, pIp->pGyroData->gyroStatus0, pIp->pGyroData->SignFlag[..];
        loop variant 9 - i;
    */
    for (i = 0; i < 9; i++)
    {
