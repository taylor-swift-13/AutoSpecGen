...
/*@ 
  ...
  requires \valid(pkv + (0..31));
  assigns pkv[0..31];
  ensures \forall integer i; 0 <= i < 32 ==> pkv[i] == 0 || pkv[i] == 0xD0 || pkv[i] == 0xC1 || pkv[i] == UI32_HIHI8(pIp->countSate) || pkv[i] == UI32_HILO8(pIp->countSate) || pkv[i] == UI32_LOHI8(pIp->countSate) || pkv[i] == UI32_LOLO8(pIp->countSate) || pkv[i] == UI32_HIHI8(tmpSate) || pkv[i] == UI32_HILO8(tmpSate) || pkv[i] == UI32_LOHI8(tmpSate) || pkv[i] == UI32_LOLO8(tmpSate) || pkv[i] == UI16_HI8(chksum) || pkv[i] == UI16_LO8(chksum);
*/
void TMProcessFun(TMProcess *pIp)
{
    ...
    /*@ 
      ...
      loop invariant \forall integer j; 0 <= j < i ==> pkv[j] == 0;
      loop assigns i, pkv[0..31];
      loop variant 32 - i;
    */
    for(i = 0; i < 32; i++)
    {
