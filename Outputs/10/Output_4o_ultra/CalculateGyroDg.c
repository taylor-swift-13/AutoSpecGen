...
/*@ 
  ...
  requires \forall int i; 0 <= i < 3 ==> \valid(pIp->pGyroData->Rtemp[i] + (0 .. 4));
  assigns tmpwa[0 .. 4];
  assigns pIp->pGyroData->W[0 .. 2];
  ensures \forall int i; 0 <= i < 5 ==> tmpwa[i] == 0 || tmpwa[i] == pIp->pGyroData->wa[pIp->pGyroData->SignFlag[i]];
...
void CalculateGyroDgFun(CalculateGyroDg *pIp)
{
    ...
    /*@ loop invariant 0 <= j <= 5;
        loop invariant \forall int i; 0 <= i < j ==> tmpwa[i] == 0;
    ...
    */
    ...
    /*@ loop invariant 0 <= j <= pIp->pGyroData->JoinTotal;
        loop invariant \forall int i; 0 <= i < j ==> tmpwa[i] == pIp->pGyroData->wa[pIp->pGyroData->SignFlag[i]];
