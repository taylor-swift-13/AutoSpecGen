...
/*@ 
  requires \valid(pIp);
  ensures pIp->mThrDistribute.stateFlag[0] == (ADDR_READ(0xE000) & 0x4) ? 1 : 0;
  ensures pIp->mThrDistribute.stateFlag[1] == (ADDR_READ(0xE000) & 0x8) ? 1 : 0;
...
void JetDataGetFun(JetDataGet *pIp)
{
    /*@ loop invariant \valid(pIp);
        loop assigns pIp->mThrDistribute.stateFlag[0], pIp->mThrDistribute.stateFlag[1];
    */
