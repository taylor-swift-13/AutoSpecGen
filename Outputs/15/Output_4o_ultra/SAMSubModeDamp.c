...
/*@ 
  ...
  ensures \old(pIp->countPublic) <= 350 ==> pIp->countPublic == \old(pIp->countPublic) + 1;
  ensures \old(pIp->countPublic) > 350 || \old(pIp->countMode) > 1000 ==> pIp->countPublic == 0 && pIp->countMode == 0 && pIp->flgMode == SAM_PITCH;
