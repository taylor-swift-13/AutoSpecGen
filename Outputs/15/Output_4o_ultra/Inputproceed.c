...
void InputproceedFun(Inputproceed *pIp)
{
    ...
    /*@ 
      ...
      assigns pIp->mAttitude.angle[0], pIp->mAttitude.angle[1];
      assigns pIp->mGyroData.Gi[0], pIp->mGyroData.Gi[1];
      ensures (flgModetmp == SAM_CRUISE && pIp->mDSSData.flgSP == 1) ==> 
              (pIp->mAttitude.angle[0] == pIp->mDSSData.royaw && pIp->mAttitude.angle[1] == pIp->mDSSData.piyaw &&
               pIp->mGyroData.Gi[0] == pIp->mDSSData.royaw && pIp->mGyroData.Gi[1] == pIp->mDSSData.piyaw);
      ensures (flgModetmp == SAM_CRUISE && pIp->mDSSData.flgSP != 1) ==> 
              (pIp->mAttitude.angle[0] == pIp->mGyroData.Gi[0] && pIp->mAttitude.angle[1] == pIp->mGyroData.Gi[1]);
      ensures (flgModetmp != SAM_CRUISE) ==> 
              (pIp->mAttitude.angle[0] == 0.0f && pIp->mAttitude.angle[1] == 0.0f);
    */
