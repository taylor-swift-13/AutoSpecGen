...
assigns Rgtrans[0..2][0..4]; // Correctly specify the range for Rgtrans
assigns Rs[0..2][0..2];
assigns RsInv[0..2][0..2];
assigns Rgtemp[0..4][0..2];
...
void CalculateGyroRsFun(CalculateGyroRs *pIp)
{
    ...
    if (pIp->pGyroData->gyroStatus0 != pIp->pGyroData->gyroStatus1)
    {
        ...
        /*@ loop invariant 0 <= j <= pIp->pGyroData->JoinTotal;
            loop assigns j, k, Rgtemp[0..pIp->pGyroData->JoinTotal-1][0..2];
        ...
        */
        ...
        /*@ loop invariant pIp->pGyroData->JoinTotal <= i <= 5;
            loop assigns i, Rgtemp[pIp->pGyroData->JoinTotal..4][0..2];
            loop variant 5 - i;
        */
