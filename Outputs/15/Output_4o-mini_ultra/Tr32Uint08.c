...
/*@ 
  ...
  ensures pIp->uiresult == \old(pIp->uiresult) || 
          (pIp->uiresult == *(pIp->pA) && *(pIp->pA) == *(pIp->pB) && *(pIp->pA) == *(pIp->pC)) ||
          (pIp->uiresult == (*(pIp->pA) & *(pIp->pB)) || (*(pIp->pA) & *(pIp->pC)) || (*(pIp->pB) & *(pIp->pC)));
...
void Tr32Uint08Fun(Tr32Uint08 *pIp)
{
    ...
    /*@ loop invariant \valid(pIp->pA) && \valid(pIp->pB) && \valid(pIp->pC);
        loop assigns uitemp1, uitemp2, uitemp3, pIp->uiresult;
    */
